...
const destBackgroundCss = path.join(targetRoot, "src/app/viana-background.css")
...
// ─── Sync background.css → viana-background.css ──────────────────────────────
//
// Writes .background-light styles to a dedicated viana-background.css file so
// that users can freely regenerate globals.css (e.g. via tweakcn) without losing
// the background-light overrides. The file must be imported in layout.tsx.
...
if (fs.existsSync(srcBackground)) {
  ...
  // Strip the file-level header comment, keep declarations only.
  ...
  const destContent =
    `/* ─────────────────────────────────────────────────────────────────────────────\n` +
    `   Viana Kit — Background Light Overrides\n` +
    `   AUTO-GENERATED by scripts/sync.js — do not edit manually.\n` +
    `   To update, run \`npm run sync\` from viana-kit-core.\n` +
    `   Import this file in your layout.tsx:\n` +
    `     import "./viana-background.css"\n` +
    `   ───────────────────────────────────────────────────────────────────────────── */\n` +
    `\n` +
    backgroundDeclarations + "\n"

  const existing = fs.existsSync(destBackgroundCss)
    ? fs.readFileSync(destBackgroundCss, "utf8")
    : ""

  if (destContent !== existing) {
    fs.writeFileSync(destBackgroundCss, destContent, "utf8")
    backgroundUpdated = true
  }

  // If globals.css still has the old injected block, strip it out.
  if (fs.existsSync(destGlobalsCss)) {
    const globalsCss = fs.readFileSync(destGlobalsCss, "utf8")
    const cleaned = globalsCss
      .replace(
    ...
    if (cleaned !== globalsCss) {
      fs.writeFileSync(destGlobalsCss, cleaned, "utf8")
    }
  }
}
...
console.log(`  background : ${backgroundUpdated ? "viana-background.css updated" : fs.existsSync(destBackgroundCss) ? "already up to date" : "skipped (srcBackground not found)"}`)
