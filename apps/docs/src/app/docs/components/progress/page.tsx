import type { Metadata } from "next";
import { CodeBlock } from "@/components/code-block";
import { ComponentPreview } from "@/components/component-preview";
import { ProgressDefaultPreview, ProgressVariantsPreview } from "@/components/previews/progress-preview";

export const metadata: Metadata = {
  title: "Progress",
};

export default function ProgressPage() {
  return (
    <article className="mx-auto max-w-3xl px-8 py-10">
      <div className="mb-8 space-y-2">
        <p className="text-sm font-medium text-primary">Components</p>
        <h1 className="text-3xl font-bold tracking-tight text-foreground">Progress</h1>
        <p className="text-lg text-muted-foreground">A component that displays the progress of an operation.</p>
      </div>

      <ComponentPreview
        preview={<ProgressDefaultPreview />}
        code={`import { AppProgress } from "@viana/ui"

export function Example() {
  return <AppProgress value={33} />
}`}
        filename="example.tsx"
      />

      <hr className="border-border my-10" />

      <section className="space-y-10">
        <div className="space-y-4">
          <h2 className="text-xl font-semibold tracking-tight text-foreground">Installation</h2>
          <CodeBlock language="bash" code="npx viana-kit add progress" />
        </div>
        <div className="space-y-4">
          <h2 className="text-xl font-semibold tracking-tight text-foreground">Import</h2>
          <CodeBlock language="tsx" code={`import { AppProgress } from "@viana/ui"AppProgress"`} />
        </div>
        <div className="space-y-4">
          <h2 className="text-xl font-semibold tracking-tight text-foreground">Usage</h2>
          <ComponentPreview
            preview={<ProgressVariantsPreview />}
            code={`<AppProgress value={0} />
<AppProgress value={33} />
<AppProgress value={66} />
<AppProgress value={100} />`}
          />
        </div>
      </section>
    </article>
  );
}